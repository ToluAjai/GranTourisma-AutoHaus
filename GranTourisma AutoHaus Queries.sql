...
-- What is the monthly revenue and the month-over-month revenue change? 
...
SELECT
    ...
    year,
    revenue,
    revenue - LAG(revenue) OVER (ORDER BY month_start) AS month_on_month_change,
    ROUND(
...
-- What is the monthly change in quantity sold? 
...
SELECT
    ...
    year,
    units_sold,
    units_sold - LAG(units_sold) OVER (ORDER BY month_start) AS month_on_month_change
FROM monthly_sales
ORDER BY month_start;

-- Which months generated the most revenue over time?
WITH monthly_revenue AS (
    SELECT
        TO_CHAR(order_date, 'Month') AS month_name,
        EXTRACT(YEAR FROM order_date) AS year,
